...
│   │   ├── chart-heads/    # Org-chart heads config (dean/directors/Carreras) — helper module, no standalone route
...
| **Chart heads** (`admin/chart-heads`)   | Org-chart heads configuration (dean/directors/Carreras) — a route-less helper module consumed by `admin/` sub-modules, not domain-defined further in code. Carreras are pre-configured per director, each resolving to `academic.programs.id`; a Program can belong to at most one school per academic period (enforced server-side). | `src/modules/admin/chart-heads/`                           |
