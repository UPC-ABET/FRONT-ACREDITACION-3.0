...
type Props = {
	...
	onClose: () => void;
	onCreate: (body: CourseSectionMaintenanceCreate) => void;
};
	...
	return (
		<Dialog
			...
			<DialogContent className="sm:max-w-3xl">
				<DialogHeader>
					<DialogTitle>{t('loads.sectionsMaintenance.create.title')}</DialogTitle>
					<DialogDescription>{t('loads.sectionsMaintenance.create.subtitle')}</DialogDescription>
				</DialogHeader>
				...
				<DialogFooter>
					...
					<Button variant="primary" onClick={handleSubmit} disabled={!canSave}>
						{saving ? t('loading.default') : t('loads.sectionsMaintenance.create.save')}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
