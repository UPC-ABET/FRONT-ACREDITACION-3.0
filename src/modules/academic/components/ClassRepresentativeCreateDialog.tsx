...
export function ClassRepresentativeCreateDialog({ saving, errorMessage, onClose, onSave }: Props) {
	const { t } = useI18n();
	...
	const handleSubmit = () => {
		...
		onSave({ sectionCode: section.sectionCode, studentCode: student.studentCode });
	};
	...
	return (
		<Dialog
			...
			<DialogContent className="sm:max-w-xl">
				<DialogHeader>
					<DialogTitle>{t('loads.classRepresentativesMaintenance.create.title')}</DialogTitle>
					<DialogDescription>
						{t('loads.classRepresentativesMaintenance.create.subtitle')}
					</DialogDescription>
				</DialogHeader>

				<div className="space-y-4">
					<LazySelect<CourseSectionMaintenanceItem>
						label={t('loads.classRepresentativesMaintenance.create.sectionLabel')}
						placeholder={t('loads.classRepresentativesMaintenance.create.sectionPlaceholder')}
					...
					<LazySelect<EnrolledStudentMaintenanceItem>
						label={t('loads.classRepresentativesMaintenance.create.studentLabel')}
						placeholder={t('loads.classRepresentativesMaintenance.create.studentPlaceholder')}
				...
				<DialogFooter>
					...
					<Button variant="primary" onClick={handleSubmit} disabled={!canSave} loading={saving}>
						{t('loads.classRepresentativesMaintenance.create.save')}
					</Button>
				</DialogFooter>
			</DialogContent>
		</Dialog>
