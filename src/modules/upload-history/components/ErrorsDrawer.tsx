...
interface ErrorsDrawerProps {
  open: boolean
  onClose: () => void
  // Excel anotado base64 que devuelve cada *UploadService cuando success=false.
  excelBase64: string | null
  // Callback opcional: re-envía la fila editada al servicio del flow (vista 4.3 §2 "Editar en caliente").
  onResubmitRow?: (row: ParsedErrorRow) => Promise<void>
}

// Drawer lateral derecho premium (blueprint §4.3.2): agrupa errores por fila y permite edición in-place.
// Por defecto solo muestra los errores; la edición/re-envío requiere `onResubmitRow` con la lógica del flow.
export default function ErrorsDrawer({ open, onClose, excelBase64, onResubmitRow }: ErrorsDrawerProps) {
  const { t } = useI18n()
  ...
  useEffect(() => {
    ...
    parse(excelBase64).then(setRows).catch(() => setRows([]))
    ...
    <div className="fixed inset-0 z-50 flex">
      ...
      <aside className="flex w-full max-w-xl flex-col overflow-y-auto border-l border-gray-200 bg-white p-6 shadow-xl">
        <header className="mb-4 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">{t('uploadHistory.drawer.title')}</h3>
        ...
        {rows.length === 0 && <p className="py-8 text-center text-sm text-gray-400">{t('uploadHistory.drawer.empty')}</p>}
                    ...
                    {Object.entries(draftCells).map(([col, value]) => (
                      <div key={col} className="flex items-center gap-2">
                        ...
                        <input
                          ...
                          value={value}
                          onChange={(e) => setDraftCells((prev) => ({ ...prev, [col]: e.target.value }))}
                        />
                      </div>
                    ))}
                    <div className="flex justify-end gap-2 pt-1">
                      <Button variant="secondary" onClick={() => setEditing(null)} disabled={resubmitting}>
                        {t('uploadHistory.drawer.cancel')}
                      </Button>
                      <Button variant="primary" onClick={() => handleResubmit(row)} disabled={resubmitting}>
                        {resubmitting ? t('uploadHistory.drawer.resubmitting') : t('uploadHistory.drawer.resubmit')}
                      </Button>
                    </div>
                  </div>
                )}
