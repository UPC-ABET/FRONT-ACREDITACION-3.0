...
export function WizardSelectEvaluatorModal({
  ...
  const availableTypes = useMemo(
    () => allTypes.filter((type) => (LIMITED_CODES.includes(type.code) ? !usedLimitedCodes.has(type.code) : true)),
    [allTypes, usedLimitedCodes],
  )

  const typeOptions = useMemo(
    () => availableTypes.map((type) => ({ label: type.name[locale as 'es' | 'en'] ?? type.name.es, value: type.id })),
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        ...
        <div className="space-y-4">
          <div className="space-y-2">
            <Input
              ...
              value={search}
              onChange={(e) => { setSearch(e.target.value); setSelectedProf(null) }}
            />
                      ...
                      <li key={p.id}>
                        ...
                        >
                          <div className="flex flex-col gap-0.5 min-w-0">
                            ...
                            {p.staff?.staff_email && (
                              <span className={`text-xs truncate ${isSelected ? 'text-zinc-300' : 'text-zinc-400'}`}>
                                {p.staff.staff_email}
                              </span>
                            )}
                          </div>
        ...
        <DialogFooter>
          ...
          <Button variant="primary" onClick={handleConfirm} disabled={!selectedProf || !selectedTypeId}>
