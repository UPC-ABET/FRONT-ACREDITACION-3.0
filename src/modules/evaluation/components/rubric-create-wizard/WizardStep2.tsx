...
export function WizardStep2({ step1, onBack, onNext }: WizardStep2Props) {
	...
	const { data: rubricTypes = [], isLoading: loadingRubric } =
		useTypesByGroupCode(RUBRIC_TYPE_GROUP);
	const { data: outcomeTypes = [], isLoading: loadingOutcome } =
		useTypesByGroupCode(OUTCOME_TYPE_GROUP);
	...
	const canContinue = !!selectedGradeType && !!determinedRubricType && !loadingMappings;
