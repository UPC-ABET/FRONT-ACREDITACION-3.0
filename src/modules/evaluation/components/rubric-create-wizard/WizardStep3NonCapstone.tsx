...
function isContinuousScores(questions: RubricQuestion[]): boolean {
  return questions.every((q) =>
    q.criteria.every((c, ci) => {
      ...
      return typeof c.minValue === 'number' && typeof prev.maxValue === 'number' && c.minValue > prev.maxValue
    })
  )
}
...
export function WizardStep3NonCapstone({
      ...
      questions.every((q) =>
        q.criteria.every(
          (c) =>
            c.minValue === '' ||
            c.maxValue === '' ||
            (typeof c.minValue === 'number' && typeof c.maxValue === 'number' && c.minValue < c.maxValue)
        )
      ),
    [questions]
  )
  ...
  const handleQuestionTextChange = (ri: number, text: string) =>
    update(questions.map((q, i) => (i === ri ? { ...q, questionText: { ...q.questionText, [locale]: text } } : q)))
  ...
  const handleCriteriaTextChange = (ri: number, ci: number, text: string) =>
    update(
      questions.map((q, i) =>
        i !== ri
          ? q
          : { ...q, criteria: q.criteria.map((c, j) => (j === ci ? { ...c, criteriaText: { ...c.criteriaText, [locale]: text } } : c)) }
      )
    )
  ...
  const handleCriteriaMinChange = (ri: number, ci: number, v: number | '') =>
    update(
      questions.map((q, i) =>
        i !== ri ? q : { ...q, criteria: q.criteria.map((c, j) => (j === ci ? { ...c, minValue: v } : c)) }
      )
    )
  ...
  const handleCriteriaMaxChange = (ri: number, ci: number, v: number | '') =>
    update(
      questions.map((q, i) =>
        i !== ri ? q : { ...q, criteria: q.criteria.map((c, j) => (j === ci ? { ...c, maxValue: v } : c)) }
      )
    )

  const validationItems = useMemo(() => {
    const items: { message: string; type: 'error' | 'warning' }[] = []
    if (!isFilled) items.push({ message: t('rubrics.editor.nonCapstone.validation.allFieldsRequired'), type: 'warning' })
    if (!continuousValid) items.push({ message: t('rubrics.editor.nonCapstone.validation.continuityRequired'), type: 'error' })
    if (!sumValid) items.push({ message: t('rubrics.editor.nonCapstone.validation.totalMustBe20').replace('{{total}}', String(total)), type: 'error' })
    if (!rangeValid) items.push({ message: t('rubrics.editor.nonCapstone.validation.minMustBeLessThanMax'), type: 'error' })
    return items
  ...
  return (
    <div className="space-y-4">
      ...
      <ValidationMessages
        ...
        successMessage={canSave ? t('rubrics.editor.nonCapstone.validation.validationComplete') : undefined}
      />
