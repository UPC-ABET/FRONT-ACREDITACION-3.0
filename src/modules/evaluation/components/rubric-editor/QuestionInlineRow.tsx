...
export function QuestionInlineRow({
  ...
  useEffect(() => {
    if (!canEdit || isTempId(question.id)) return
    if (debounceRef.current) clearTimeout(debounceRef.current)
    debounceRef.current = setTimeout(() => { void flushPatch() }, 800)
  ...
  return (
    <div
      className={`flex flex-wrap items-center gap-3 rounded-lg border px-3 py-2 transition-colors ${trashHovered
          ? 'border-red-200 bg-red-50'
          : 'border-zinc-100 bg-zinc-50/80'
        }`}
    >
      <span
        className={`shrink-0 text-xs font-semibold uppercase tracking-wide transition-colors ${trashHovered ? 'text-red-400' : 'text-zinc-500'
          }`}
      >
        {labelPrefix} {index + 1}
      </span>
