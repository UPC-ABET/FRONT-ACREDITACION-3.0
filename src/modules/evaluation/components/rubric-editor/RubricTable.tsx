        ...
        <thead>
          <tr className="border-b border-zinc-200 bg-zinc-50">
            ...
            {Array.from({ length: columnCount }).map((_, colIndex) => (
              <th
                key={colIndex}
                className={`w-[200px] border-l border-zinc-100 p-2 text-left text-xs font-semibold uppercase tracking-wide text-zinc-600 transition-colors duration-300 ${hoveredCol === colIndex ? 'bg-red-50' : ''
                  }`}
              >
                <div className="flex items-center justify-between gap-2">
                  ...
                  {canEdit && columnCount > 1 ? (
                    ...
                    >
                      <TrashIcon className={`h-3.5 w-3.5 transition-colors duration-300 ${hoveredCol === colIndex ? 'text-red-500' : ''}`} />
                    </button>
                  ) : null}
                </div>
              </th>
        ...
        <tbody>
          {questions.map((question, rowIndex) => (
            ...
            >
              {canEdit ? (
                <td
                  className={`w-8 border-r border-zinc-100 align-middle transition-colors duration-300 ${hoveredRow === rowIndex ? 'bg-red-50' : ''
                    }`}
                >
                  {questions.length > 1 ? (
                    <div className="flex items-center justify-center h-fit py-3 ">
                      ...
                      >
                        <TrashIcon className={`h-4 w-4 transition-colors duration-300 ${hoveredRow === rowIndex ? 'text-red-500' : ''}`} />
                      </button>
                    </div>
                  ) : null}
                </td>
              ...
              {/* Question cell */}
              <td className={`align-top transition-colors duration-300 ${hoveredRow === rowIndex ? 'bg-red-50' : ''}`}>
                <QuestionCell
              ...
              {Array.from({ length: columnCount }).map((_, colIndex) => {
                ...
                return (
                  <td
                    ...
                    className={`align-top border-l border-zinc-100 transition-colors duration-300 ${isColHovered || isRowHovered ? 'bg-red-50' : ''
                      }`}
                  >
                    {cell ? (
                      <CriteriaCell
