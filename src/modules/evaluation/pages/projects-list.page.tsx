...
import { getSchoolIdFromToken } from '@/shared/lib/jwt'
import { academicPeriodsService, programsService, coursesService } from '@/modules/academic/services'
...
export function ProjectsListPage() {
  ...
  const { data: programs = [] } = useQuery({
    queryKey: ['programs', 'filtered', { school_id: schoolId, academic_period_id: selectedPeriod?.value, is_active: true }],
    queryFn: () =>
      programsService
        .getByFilters({ school_id: schoolId!, academic_period_id: selectedPeriod!.value, is_active: true })
        .then((r) => r.data),
    enabled: !!selectedPeriod && !!schoolId,
  })

  const { data: courses = [] } = useQuery({
    queryKey: ['courses', 'filtered', { school_id: schoolId, academic_period_id: selectedPeriod?.value, program_id: selectedProgram?.value, is_active: true }],
    queryFn: () =>
      coursesService
        .getByFilters({
          school_id: schoolId!,
  ...
  const { data: projects = [], isLoading, isError, error } = useQuery({
    queryKey: ['projects', 'filtered', { school_id: schoolId, academic_period_id: selectedPeriod?.value, program_id: selectedProgram?.value, course_id: selectedCourse?.value }],
    queryFn: () =>
      projectsService
        .getByFilters({
          school_id: schoolId!,
        ...
        {(selectedPeriod || selectedProgram || selectedCourse) && (
          <div className="flex justify-end">
            <button
              ...
              className={cn(buttonVariants({ variant: 'warning', size: 'md' }), "inline-flex items-center gap-2 rounded-lg border border-red-200 bg-white px-4 py-2 text-sm font-medium text-zinc-700 transition-colors hover:bg-red-100 hover:text-red-500")}
            >
