...
function unwrapApiData<T>(response: unknown): T | null {
  ...
  if (data && typeof data === 'object' && Object.prototype.hasOwnProperty.call(data as Record<string, unknown>, 'data')) {
...
export function useRubricEditor({ rubricId, initialRubric }: UseRubricEditorOptions) {
  ...
  console.debug('[useRubricEditor] init', { rubricId, hasInitialRubric })

  const query = useQuery<RubricDetail>({
    queryKey: rubricEditorQueryKeys.detail(rubricId),
    queryFn: async (): Promise<RubricDetail> => {
      console.debug('[useRubricEditor] queryFn start', { rubricId })
      if (initialRubric) {
        console.debug('[useRubricEditor] using initialRubric')
      ...
      console.debug('[useRubricEditor] performance levels response', levelsRes)
      ...
      const outcomesById = new Map<number, any>(
        (data.outcomes ?? []).map((o: any) => [o.id, o])
      )
      ...
      const commissions = (data.commissions ?? []).map((commission: any) => {
        ...
        const outcomes = outcomeIdsList.map((outcomeId: number) => {
        ...
        const commissionName = typeof nameRaw === 'string'
      ...
      const gradeType = typeof gradeTypeName === 'string'
        ? { en: gradeTypeName, es: gradeTypeName }
        : (gradeTypeName ?? { en: '', es: '' })

      // ── Program (top-level in new API) ───────────────────────────────────────
      const prog = data.program
      const programName = typeof prog?.name === 'string'
        ? { en: prog.name, es: prog.name }
        : (prog?.name ?? { en: '', es: '' })

      // ── Course name ──────────────────────────────────────────────────────────
      const courseName = typeof data.course?.name === 'string'
