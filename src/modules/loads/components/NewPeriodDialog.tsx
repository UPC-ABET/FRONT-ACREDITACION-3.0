...
export default function NewPeriodDialog({ open, onClose, onCreated }: NewPeriodDialogProps) {
	...
	const handleSubmit = () => {
		...
		if (!PERIOD_CODE_PATTERN.test(code)) {
			setError(t('loadsSetup.period.error.codeFormat'));
			return;
		}
		if (!startDate || !endDate) {
			setError(t('loadsSetup.period.error.datesRequired'));
			return;
		}
		if (new Date(endDate) < new Date(startDate)) {
			setError(t('loadsSetup.period.error.endBeforeStart'));
			return;
		}
		...
		create.mutate(
			{ code, start_date: startDate, end_date: endDate, modality_code: modalityCode },
			{
				...
				onError: (err) => setError(err.message || t('loadsSetup.period.error.generic')),
			},
		);
	};
	...
	return (
		<Dialog
			...
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{t('loadsSetup.period.dialogTitle')}</DialogTitle>
				</DialogHeader>
				<div className="space-y-4">
					<div>
						<label htmlFor="period-code" className="mb-1 block text-xs font-medium text-gray-600">
							{t('loadsSetup.period.codeLabel')}
						</label>
						<Input
							id="period-code"
							value={code}
							...
							placeholder="2026-01"
						/>
						<p className="mt-1 text-xs text-gray-400">{t('loadsSetup.period.codeHint')}</p>
					</div>
					...
					<div className="grid grid-cols-2 gap-3">
						<div>
							<label
								...
								{t('loadsSetup.period.startLabel')}
							</label>
							<Input
								id="period-start"
								type="date"
					...
					<div>
						<label
							htmlFor="period-modality"
							className="mb-1 block text-xs font-medium text-gray-600">
							{t('loadsSetup.period.modalityLabel')}
						</label>
						<Select
							name="period-modality"
							value={{
								value: modalityCode,
								label: t(MODALITY_LABEL_KEYS[modalityCode]),
							}}
							onChange={(_name, opt) => {
								const v = Array.isArray(opt) ? opt[0] : opt;
								if (v) setModalityCode(String(v.value) as PeriodModalityCode);
							}}
							options={PERIOD_MODALITY_CODES.map((c) => ({
								value: c,
								label: t(MODALITY_LABEL_KEYS[c]),
							}))}
						/>
					</div>
					...
					<div className="flex justify-end gap-2 pt-2">
						<Button variant="secondary" onClick={onClose} disabled={create.isPending}>
							{t('loadsSetup.period.cancel')}
						</Button>
						<Button variant="primary" onClick={handleSubmit} disabled={create.isPending}>
							{create.isPending ? t('loadsSetup.period.creating') : t('loadsSetup.period.create')}
						</Button>
					</div>
				</div>
			</DialogContent>
