...
export default function UploadCanvas({ flow, onSubmit, onBannerScrap }: UploadCanvasProps) {
	...
	if (!flow) {
		return (
			<Card
				title={t('uploads.canvas.upload.title')}
				description={t('uploads.canvas.upload.pickFlow')}>
				<p className="py-6 text-center text-sm text-gray-500">
					{t('uploads.canvas.upload.noFlowSelected')}
				</p>
			</Card>
		);
	}
	...
	return (
		<Card
			title={`${t('uploads.canvas.upload.title')} — ${t(flow.displayKey)}`}
			description={interpolate(t('uploads.canvas.upload.dragHintWith'), {
				flow: t(flow.displayKey),
			})}>
			<div className="space-y-4">
				<label
					...
					<span className="text-sm font-medium text-gray-700">
						{file ? file.name : t('uploads.canvas.upload.dropzone')}
					</span>
				...
				{isInvalid && (
					<div className="rounded-md border border-red-300 bg-red-50 px-3 py-2">
						<p className="text-sm font-medium text-red-700">
							{t('uploads.canvas.upload.invalidHeaders')}
						</p>
						<p className="mt-1 text-xs text-red-600">
							{t('uploads.canvas.upload.missingColumns')}: {validation?.missing.join(', ')}
						</p>
					</div>
				)}

				{validation?.ok && (
					<p className="text-sm text-green-700">✓ {t('uploads.canvas.upload.headersValid')}</p>
				)}

				<div className="flex justify-between gap-3">
					{flow.canBannerScrap && onBannerScrap && (
						<Button variant="secondary" onClick={() => setBannerOpen(true)} disabled={submitting}>
							⚡ {t('uploads.canvas.banner.pullFromBanner')}
						</Button>
					)}
					<Button variant="primary" onClick={handleSubmit} disabled={!validation?.ok || submitting}>
						{submitting ? t('uploads.canvas.upload.submitting') : t('uploads.canvas.upload.submit')}
					</Button>
				</div>
			</div>
