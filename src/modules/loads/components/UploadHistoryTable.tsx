...
export default function UploadHistoryTable({
	...
	return (
		<Card title={t('uploadHistory.table.title')} description={t('uploadHistory.table.description')}>
			{isLoading && (
				<p className="py-4 text-sm text-gray-500">{t('uploadHistory.table.loading')}</p>
			)}
									...
									<td className="py-2">
										<span
											className={`rounded-full px-2 py-0.5 text-xs ${
												STATUS_COLOR[log.status] ?? 'bg-gray-100 text-gray-600'
											}`}>
											{log.status}
										</span>
									</td>
									<td className="py-2 max-w-[200px] truncate text-xs text-gray-600">
										{log.source_file ?? '—'}
									</td>
									<td className="py-2 text-right text-xs">
										{log.loaded_rows ?? 0} / {log.total_rows ?? 0}
										{log.error_rows ? (
											<span className="ml-1 text-red-600">({log.error_rows})</span>
										) : null}
									</td>
