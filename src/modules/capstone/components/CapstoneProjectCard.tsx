...
// Tarjeta de proyecto Capstone con círculo de progreso SVG (blueprint §4.4.1).
export default function CapstoneProjectCard({ project, selected, onSelect }: CapstoneProjectCardProps) {
  const { t } = useI18n()
  const pct = project.total_students > 0 ? Math.round((project.graded_students / project.total_students) * 100) : 0
  const radius = 22
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 text-xs text-gray-600">
        {project.graded_students} {t('capstone.card.gradedOf')} {project.total_students} {t('capstone.card.students')}
      </p>
    </button>
  )
}
