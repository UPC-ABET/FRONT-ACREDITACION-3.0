...
**Superseded 2026-08-21 — see `proposal.md` § Scope extension.** The two `stats.counts`
snapshots above are a point-in-time reading, not the current shape: the backend renamed
these fields to English in a follow-up (independently re-confirmed against
`scraper.service.ts`/`planner-scraper.service.ts` on `develop` before implementing).
Current shape: Banner `{ schedule, enrollment, students, grades }`, Planner
`{ sections, evaluations, grades }` — see `tasks.md` § Unplanned, Tasks U.1/U.2. This is
exactly the drift the § Risks row below predicted; treat that row as materialized once,
...
| `stats`/`counts` internal shape (`nota` field, etc.) isn't spec-tracked (`unknown` in `openapi.json`) — a future backend change to that shape wouldn't be caught by `/abet-verify-contract`. **Materialized 2026-08-21**: the backend renamed the whole `counts` shape a second time within the same week, unannounced by any spec change — see `proposal.md` § Scope extension. | Out of scope to fix backend spec coverage in this change; flagged here and in `proposal.md` so it isn't assumed covered going forward. Given it's now a demonstrated recurring pattern rather than theoretical, worth raising with the backend team as a candidate for typing `counts` in `openapi.json`. |
| Only the `grades`/`nota` count field got a defensive `?? 0` fallback (added by the first audit round, after it was the one field observed missing from a real payload) — the sibling fields (`schedule`/`enrollment`/`students`, `sections`/`evaluations`) are read unguarded from the same untyped object, an inconsistent risk posture flagged by the second audit round.      | Applied `?? 0` uniformly to every `ScrapeCounts`/`PlannerScrapeCounts` field at every read site, not just `grades` — see `tasks.md` § Unplanned, Task U.4.                                                                                                                                                |
