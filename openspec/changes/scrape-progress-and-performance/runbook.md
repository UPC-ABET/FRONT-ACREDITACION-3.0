...
| 4   | Open `ScrapeRunHistory` / `PlannerScrapeRunHistory` while at least one row is `running`                                      | Status Badge only, no phase label anywhere in the row — reverted 2026-08-21, see `proposal.md` § Scope reduction                                            |
| 5   | Let a run reach a terminal status (`completed`/`partial`/`failed`/`expired`)                                                 | The last-known phase label is still shown on the progress card — not hidden                                                                                 |
| 6   | Force an unrecognized `phase` value via browser devtools' network response override (or a local mock) on one status response | The raw string renders in place of a translated label; no console error, no crashed component                                                               |
...
| Phase label appears in the history table                         | A regression of the 2026-08-21 revert — the `status` cell in `ScrapeRunHistory.tsx`/`PlannerScrapeRunHistory.tsx` should render only the `Badge`, no `ScrapePhaseLabel`/`PlannerScrapePhaseLabel` | Check the column `cell` definition against `tasks.md` § Unplanned, Task U.1           |
