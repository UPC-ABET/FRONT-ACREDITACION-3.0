...
| **(Added, post-implementation audit)** `useConfigureChartHeads` has no optimistic-concurrency protection — two admins/tabs saving the chart-heads form concurrently get last-write-wins, and this PR widens the blast radius since a whole director's `programs[]` can now be lost in one overwrite, not just director-level fields. Pre-existing pattern, not introduced by this PR, but worth surfacing now that the risk is larger.         | Not fixable from the frontend alone — needs backend support (a version/ETag/If-Match token on `POST /admin-chart-heads/configure`). Recorded here as a follow-up rather than silently dropped; not blocking this PR.                       |
