...
```
None for this frontend repo directly — but this PR must not merge until the backend
...
(not just the feature branch) immediately before opening this PR — see
docs/CONTEXT.md's cross-repo sequencing rule.

Manual end-to-end verification below additionally requires a locally-runnable backend on
that branch (or a shared environment running it) — verifying against the feature branch
alone is sufficient for development, but re-verify against `staging` before merge.
```
...
- Do not merge this PR before the backend change (PR #107) has reached `staging` in
  `UPC-ABET/BACK-ACREDITACION-3.0` — frontend code must never reach production ahead of the
  API it depends on (`docs/CONTEXT.md`'s sequencing rule).
